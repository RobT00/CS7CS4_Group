...
class SMOGN:
    ...
    def __init__(
        self,
        threshold=0.9,
        over_sampling_ratio=0.1,
        under_sampling_ratio=1.0,
        k=10,
        relevanse_base=0.5,
        pert=0.02,
        metric="minkowski",
    ...
    def fit_transform(self, X, target_column):
        ...
        """
        self.relevances = self.relevance_fn(
            X[target_column].values, self.relevance_base
        )
        ...
        if self.under_sampling_ratio < 1.0:
        ...
        if self.over_sampling_ratio > 0.0:
            ...
            for i in range(sample_num):
                ...
                maxD = np.median(dist[idx]) / 2.0
                std = np.std(X2)
                new_sample = pd.Series([0.0] * len(X.columns), index=X.columns)
                for ci, col in enumerate(X2):
                    if col in categorical_columns:
                        ...
                        if dist_i < maxD:
                            ...
                            new_sample[col] = (
                                rare_sample[col] + np.random.randn() * std[ci] * pert
                            )
    ...
    def relevance_fn(self, y, k=0.5, eps=1e-8):
        ...
        m = (np.abs(y_tilda - (y[adjL] + i)) + np.abs(y_tilda - (y[adjH] - d))) / 2.0
        ...
        c1 = np.exp((xa * np.log(1.0 / 3.0)) / (xa - xb))
        c2 = np.exp((xc * np.log(1.0 / 3.0)) / (xc - xd))

        s1 = np.log(1.0 / 3.0) / (xa - xb)
        s2 = np.log(1.0 / 3.0) / (xc - xd)
        relevances = np.where(
            y <= y_tilda, c1 / (c1 + np.exp(s1 * y)), c2 / (c2 + np.exp(s2 * y))
        )
        return relevances
